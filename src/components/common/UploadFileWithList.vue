        ...
        <a-table :columns="uploadColumns" :data-source="uploadData" :scroll="{ x: true }"
            ...
            <template #bodyCell="{ column, record, text ,index }">
                ...
                <template v-if="column.key === 'amount'">
                    <a-input-number v-model:value="record.amount" style="width: 66px;" :min="1" :precision="0" placeholder="请输入"/>
                    {{ $t('in.item') }}
                </template>
                <template v-if="column.key === 'discount'">
                    <a-input-number v-model:value="record.discount" style="width: 66px;" :min="1" :max="100" :precision="0" placeholder="请输入"/> %
                </template>
                ...
                <template v-if="column.dataIndex === 'operation'">
                    <a-button type="link" class="danger" @click="handleFileRemove(record.id, index)"><i
