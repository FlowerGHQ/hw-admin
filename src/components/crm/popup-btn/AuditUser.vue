<template>
    <a-steps :current="999">
        <a-step v-for="(item,index) in auditUserList"  >
            ...
            <template #title>
                {{item.name}}
                <a-button type="link" class="danger" @click="handleDeleteCustomer(index)" v-if="$auth('crm-bo.save')"><i
                    class="icon i_close_c"/>
                </a-button>
            </template>
        </a-step>
        ...
        <a-step status="finish">
            <template #icon>
                <UserSelect @select="handleAddCustomerShow" :group-id="groupId" :radioMode="true" btn-class="select-item-btn" btnType='link' :btnText="btnText">
