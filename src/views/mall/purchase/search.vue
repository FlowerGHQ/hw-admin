            ...
            <a-spin tip="Loading..." :spinning="spinning">
                <div class="body">
                    ...
                    <div class="list-body" v-if="vehicleList.length !== 0">
                        <p class="list-body-title">{{ $t('mall.vehicle_models') }}({{ vehicleList.length }})</p>
                    ...
                    <div class="list-body" v-if="sparepartsList.length !== 0">
                        <p class="list-body-title">{{ $t('mall.accessories') }}({{ sparepartsList.length }})</p>
                    ...
                    <div class="list-body" v-if="peripheralList.length !== 0">
                        <p class="list-body-title">{{ $t('mall.peripheral_products') }}({{ peripheralList.length }})</p>
                    ...
                    <div class="list-body" v-if="listNews.length !== 0">
                        <p class="list-body-title">{{ $t('mall.news') }}({{ listNews.length }})</p>
                            ...
                            <div class="mes">
                                <div class="mes-left">
                                    <div class="text">
                                        ...
                                        <p class="code" :title="item.description">
...
const getCarList = () => {
    ...
    // 如果有正在执行的搜索 则先添加搜索队列
    if (spinning.value)
        return fetchList.unshift({
            fn: searchListFetch,
            params: { ...params },
        });
    spinning.value = true;
    searchListFetch({ ...params })
        .then(res => {
...
const executeFetchList = () => {
    ...
    // 执行最新的一个接口
    fn(fetchList[0].fn, fetchList[0].params)
        .then(result => {
            const res = result;
            list.value = res?.merge_maps.list;
            listNews.value = res?.post_list.list;
