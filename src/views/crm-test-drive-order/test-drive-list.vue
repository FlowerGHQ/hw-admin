...
export default {
  ...
  methods: {
    ...
    getTableData() {
      ...
      searchForm.begin_time = searchForm.begin_time
        ? dayjs(searchForm.begin_time).unix()
        : 0; // 日期转时间戳
      searchForm.end_time = searchForm.end_time
        ? dayjs(searchForm.end_time).unix()
        : 0; // 日期转时间戳
      this.loading = true;
    ...
    getTableTimeData() {
      ...
      searchForm.begin_time = Math.round(
        this.calendarTime.getTime() / 1000 - 60 * 24 * 3600
      );
      searchForm.end_time = Math.round(
        this.calendarTime.getTime() / 1000 + 60 * 24 * 3600
      );
    ...
    getCalendarDate(e) {
      ...
      let begin_time = Math.round(
        new Date(new Date(e).setHours(0, 0, 0, 0)).getTime() / 1000
      );
      let end_time = Math.round(
        new Date(new Date(e).setHours(23, 59, 59, 999)).getTime() / 1000
      );
      // this.createTime[0] = Math.round(new Date(e).getTime())
      // this.createTime[1] = Math.round(new Date(e).getTime())
      let createTime = [];
      createTime[0] = begin_time
        ? dayjs.unix(begin_time).format("YYYY-MM-DD HH:mm:ss")
        : undefined;
      createTime[1] = end_time
