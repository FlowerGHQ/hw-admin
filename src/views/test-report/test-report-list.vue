        ...
        <div class="search-container" v-if="$auth('ADMIN')">
            <a-row class="search-area">
                <a-col :xs='24' :sm='24' :xl="8" :xxl='8' class="search-item">
                    ...
                    <div class="value">
                        <a-input v-model:value='searchForm.device_uid' :placeholder="$t('def.input')" @keydown.enter="handleSearch()"/>
                    </div>
                </a-col>
                <a-col :xs='24' :sm='24' :xl="8" :xxl='8' class="search-item">
                    <div class="key">测试类型:</div>
                    <div class="value">
                        <a-select v-model:value='searchForm.type' :placeholder="$t('def.select')" @change="handleSearch()" allow-clear>
                            <a-select-option v-for="item of TYPE_LIST" :key="item.value" :value="item.value" :label="item.text">{{item.text}}</a-select-option>
                        </a-select>
                    </div>
                </a-col>
                <a-col :xs='24' :sm='24' :xl="8" :xxl='8' class="search-item">
                    ...
                    <div class="value">
                        <a-select v-model:value='searchForm.status' :placeholder="$t('def.select')" @change="handleSearch()" allow-clear>
                            <a-select-option v-for="item of STATUS_LIST" :key="item.value" :value="item.value" :label="item.text">{{item.text}}</a-select-option>
                        </a-select>
                    </div>
                </a-col>
            </a-row>
