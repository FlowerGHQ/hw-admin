<template>
    <div>
        <div class="classes">
            <p class="title">班次管理<a-button type="primary" style="float: right;" @click="creatTemplate">创建班次模板</a-button></p>
            <div class="select">
            ...
            <div class="drag">
                <template v-for="(item,index) in arr" :key="item.id">
                    <div class="drag-box" :id="index" draggable="true" @dragstart="(event) => dragStart(event, item)" @dragend="dragend">
        ...
        <div class="template">
            ...
            <div class="template-container">
                <div :class="`template-container-item template-container-item${item.id}`" v-for="(item, index) in templateNum" :key="index" @dragleave="e => dragLeave(e, item.id)" @dragenter="e => dragEnter(e, item.id)" @dragover="dragOver" @drop="e => drop(e, index)">
                    <div class="time">
                        <div class="time-title">
                            <div class="time-title-text">{{ item.title }}<img class="time-title-edit" src="@/assets/images/retail-crm/edit.png" @click="edit(item.id, item.title)"></div>
                            <div class="time-title-delete" @click="deleteTemplate(item.id)"><img class="time-title-deleteImg" src="@/assets/images/retail-crm/delete.png">删除</div>
                        </div>
                        <div class="time-container">
                            <div class="time-item" :style="{ background: Core.Const.RETAIL_CRM.TIME_MAP[itemTime.name].background }" v-for="(itemTime, indexTime) in item.timeList" :key="index">
                                <div class="time-item-top" :style="{ background: Core.Const.RETAIL_CRM.TIME_MAP[itemTime.name].color }"></div>
                                <p class="time-item-name" :style="{ color: Core.Const.RETAIL_CRM.TIME_MAP[itemTime.name].color }">{{ itemTime.name }}</p>
                                <a-time-range-picker style="background-color: #FFF !important;" format="HH:mm" :bordered="false"  v-model:value="itemTime.time"/>
                            </div>
                        </div>
                    </div>
                    <div class="drop">
                        ...
                        <div :class="`drop-box drop-box-${index}`">
                            <div class="drag-box" v-for="(itemStore, indexStore) in item.storeList" :key="itemStore.id" draggable="true" @dragstart="(event) => dragStart(event, itemStore)" @dragend="dragend">
                                <img src="@/assets/images/retail-crm/store-image.png">
                                <p class="drag-box-name">{{ itemStore.name }}</p>
                                <p class="drag-box-area">{{ itemStore.name }}<div style="float: right;" class="delete" @click="deleteStore(index, itemStore.id)"></div></p>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    methods:{
        ...
        drop(event, index) {
            ...
            if (this.startIndex || this.startIndex === 0) {
                this.templateNum[this.startIndex].storeList = this.templateNum[this.startIndex].storeList.filter(a => a.id !== data.id)
            }
