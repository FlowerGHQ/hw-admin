<template>
  ...
  <template v-if="isDrawer && !showAddPop" class="modal-container">
    ...
    >
      ...
      <template #footer>
        <a-button type="primary" @click="saveRegional">{{
          $t("retail.save")
        }}</a-button>
      </template>
    </a-drawer>
  ...
  <template v-if="showAddPop">
    <a-modal
      v-model:visible="showAddPop"
      :title="$t('crm_st.add_city')"
      :width="360"
      @ok="addCityOk"
    >
      <div class="aa">
  ...
  computed: {
    ...
    nowAddressList() {
      ...
      this.addressOptions.forEach((el1) => {
        ...
        if (
          el1.children.filter((el2) => this.checkedKeys.includes(el2.name))
            .length > 0
        )
          list.push({
            key: el1.name,
            name: el1.name,
            children: el1.children.filter((el2) =>
              this.checkedKeys.includes(el2.name)
            ),
          });
      });
      return list;
