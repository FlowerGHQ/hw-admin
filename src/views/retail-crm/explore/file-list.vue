<template>
  <div id="file-list" class="list-container">
    <div class="left">
      ...
      <div class="file-box">
        <div class="search-item">
          <!-- 搜索文件 -->
          <a-input :placeholder="$t(/* 搜索文件 */'s.search_place')" v-model:value="searchName" @pressEnter="searchCity" />
        ...
        <div class="file-explore-box">
          <div class="explore-list" :class="{ 'explore-list-click': index === 3 }" v-for="(fileItem, index) in fileListNow" :key="fileItem.id" @contextmenu.prevent="showContextMenu(fileItem, index, $event, 'file')" @click="fileItemSelect(fileItem.id)" >
            <div class="file-left">
              ...
              <a-tooltip placement="top" :title="fileItem.fileName">
                ...
                <a-input v-else class="name-box name-input" v-focus :placeholder="$t('s.search_place')" v-model:value="fileItemCon.name" @blur="blurFileName" />
              </a-tooltip>
            </div>
    ...
    <div style="flex: 1">
      <div class="title-container">
        <div class="title-area">{{ $t(/* 目录 */"n.content") }}
      ...
      <div class="file-content">
        <div class="content-list" v-for="(contentItem, index) in fileContentList" @contextmenu.prevent="showContextMenu(contentItem, index, $event, 'content')" >
          <div class="img-box">
            ...
            <div class="to-top-img">
              <a-tooltip placement="top" :title="$t('retail.to_top')"  v-if="true">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                      <path fill-rule="evenodd" clip-rule="evenodd" d="M10.9091 7.66683L8.66683 7.66683L8.66683 14.3335L7.33349 14.3335L7.33349 7.66683L5.09124 7.66683C4.8012 7.66683 4.64954 7.32206 4.84552 7.10825L7.75444 3.93489C7.88655 3.79076 8.11376 3.79076 8.24588 3.93489L11.1548 7.10825C11.3508 7.32206 11.1991 7.66683 10.9091 7.66683ZM14.3335 3.00016L14.3335 1.66683L1.66683 1.66683L1.66683 3.00016L14.3335 3.00016Z" fill="#86909C"/>
                </svg>
              </a-tooltip>
              ...
              <a-tooltip placement="top" :title="$t('retail.cancle_top')" v-else>
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                      <g clip-path="url(#clip0_743_22633)" >
                          <rect y="2.06055" width="1.5" height="19.7013" transform="rotate(-45 0 2.06055)" fill="#86909C"/>
                          <path fill-rule="evenodd" clip-rule="evenodd" d="M3.60889 1.6665L14.3335 1.6665L14.3335 2.99984L4.94222 2.99984L3.60889 1.6665ZM3.06966 2.99984L1.73633 1.6665L1.66683 1.6665L1.66683 2.99984L3.06966 2.99984ZM6.85651 4.91412L7.75444 3.93456C7.88655 3.79044 8.11377 3.79044 8.24588 3.93456L11.1548 7.10793C11.3508 7.32173 11.1991 7.6665 10.9091 7.6665L9.60889 7.6665L6.85651 4.91412ZM8.66683 8.597L5.96094 5.89111L4.84552 7.10793C4.64954 7.32173 4.8012 7.6665 5.09124 7.6665L7.3335 7.6665L7.3335 14.3332L8.66683 14.3332L8.66683 8.597Z" fill="#86909C"/>
                      </g>
                      <defs>
          ...
          <a-tooltip placement="top" :title="contentItem.fileName">
            ...
            <a-input v-else class="name-box name-input" v-focus :placeholder="$t('s.search_place')" v-model:value="fileContentObj.name" @blur="blurFileContent" />
          </a-tooltip>
            ...
            <div class="right-click">
              <a-tooltip placement="top" :title="$t('retail.delete')">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none" class="cursor-pointer" >
                  <path class="btn" fill-rule="evenodd" clip-rule="evenodd" d="M11.3332 2.99984V1.6665L4.6665 1.66651V2.99984L1.6665 2.99984V4.33317H2.83317L2.83317 13.3332C2.83317 13.8855 3.28089 14.3332 3.83317 14.3332L12.1665 14.3332C12.7188 14.3332 13.1665 13.8855 13.1665 13.3332V4.33317L14.3332 4.33317V2.99984L11.3332 2.99984ZM4.1665 12.9998L4.1665 4.33317L11.8332 4.33317V12.9998L4.1665 12.9998ZM5.99984 5.99984V10.9998H7.33317V5.99984H5.99984ZM8.6665 5.99984V10.9998H9.99984V5.99984H8.6665Z" fill="#86909C" />
                </svg>
              </a-tooltip>
              <a-tooltip placement="top" :title="$t('retail.re_name')">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none" class="write-name" @click="writeName" >
                  <path class="btn" fill-rule="evenodd" clip-rule="evenodd" d="M9.26625 1.62981C9.65677 1.23928 10.2899 1.23928 10.6805 1.62981L12.5424 3.4917C12.9329 3.88223 12.9329 4.51539 12.5424 4.90592L5.72573 11.7225C5.5666 11.8817 5.35844 11.9825 5.13491 12.0087L3.11926 12.2446C2.49464 12.3178 1.95836 11.803 2.00583 11.1759L2.16504 9.07261C2.18309 8.83416 2.28599 8.61007 2.45508 8.44098L9.26625 1.62981ZM8.76919 4.01248L10.1597 5.40298L11.3638 4.19881L9.97336 2.80832L8.76919 4.01248ZM9.21688 6.34578L7.82638 4.95529L3.48524 9.29643L3.36588 10.8733L4.86483 10.6978L9.21688 6.34578ZM14 14.6664L2 14.6664V13.333L14 13.333V14.6664Z" fill="#86909C" />
                </svg>
              </a-tooltip>
            </div>
          </div>
    ...
    <!-- 右键弹框 -->
    <div class="triangle-pop" v-if="contextMenuVisible" :style="{ top: contextMenuY - 80 + 'px', left: contextMenuX - 220 + 'px', }" >
