      ...
      <div class="btns-area">
        <template v-if="detail.status === STATUS.POOL">
          ...
          <a-button
            ...
            v-if="$auth('crm-customer.save') && !isMoreCss"
            >{{ $t("n.edit") }}</a-button
          >
          <!-- 领取 -->
          <a-button
          ...
          <a-button
            type="primary"
            @click="handleBatch('distribute')"
            v-if="$auth('crm-customer.distribute')">
            {{ $t("crm_c.distribute") }}
          </a-button>
          <!-- 删除 -->
          <a-button
        ...
        <template
          ...
          <template
            ...
            <a-button
              type="primary"
              @click="handleBatch('transfer')"
              v-if="$auth('crm-customer.transfer')">
              {{ $t("crm_c.transfer") }}
            </a-button>
            <!-- 退回 -->
            <a-button
              type="danger"
              @click="handleReturnPool"
          ...
          <a-row class="desc-detail">
            <a-col :xs="24" :sm="12" :lg="8" class="detail-item">
              ...
              <span class="value phone-hover">
                {{
                  detail.phone?.includes(detail.phone_country_code)
                    ? ""
                    : detail.phone
                }}
                {{ detail.phone }}
            ...
            <a-col :xs="24" :sm="12" :lg="8" class="detail-item">
              ...
              <span class="value">{{
                detail.own_user ? detail.own_user.name : "-"
              }}</span>
            </a-col>
            <a-col :xs="24" :sm="12" :lg="8" class="detail-item">
            ...
            <a-col :xs="24" :sm="12" :lg="8" class="detail-item">
              ...
              <span class="value">
                {{
                  $Util.orderTestSourceType(
                    detail.crm_test_drive_order?.channel,
                    $i18n.locale
                  ) || "-"
                }}
              </span>
            ...
            <a-col :xs="24" :sm="24" :lg="24" class="detail-item">
              ...
              <span
                class="value"
                style="overflow: initial; white-space: normal">
                <LabelList
                  :targetId="id"
                  :targetType="Core.Const.CRM_LABEL.CATEGORY.CUSTOMER" />
              </span>
            </a-col>
            <!-- 门店邮件是否发送 -->
            <a-col :xs="24" :sm="12" :lg="8" class="detail-item">
              <span style="color: #8b9aae"
                >{{ $t("dis.store_is_send_mail") }}：</span
              >
              <span>
                <span v-if="!detail.crm_test_drive_order?.flag_mail_sent_store"
                  >-</span
                >
                <!-- 1已发 2未发-->
                <span
                  v-else-if="
                    detail.crm_test_drive_order?.flag_mail_sent_store == 1
                  ">
                  {{ $t("dis.been_sent") }}
                  <!-- ({{ $t('dis.mail') }}:1234) -->
                </span>
                <span
                  v-else-if="
                    detail.crm_test_drive_order?.flag_mail_sent_store == 2
                  ">
                  <span style="color: #f31c12">{{ $t("dis.fail_send") }}</span>
                  <a-button
                    type="link"
                    style="margin-left: 8px"
                    @click="resetEmailEvent('store')"
                    >{{ $t("dis.fagain_send") }}</a-button
                  >
                </span>
              </span>
            </a-col>
            ...
            <a-col :xs="24" :sm="12" :lg="8" class="detail-item">
              <span style="color: #8b9aae"
                >{{ $t("dis.user_is_send_mail") }}：</span
              >
              <span>
                <span v-if="!detail.crm_test_drive_order?.flag_mail_sent_user"
                  >-</span
                >
                <!-- 1已发 2未发 -->
                <span
                  v-else-if="
                    detail.crm_test_drive_order?.flag_mail_sent_user == 1
                  ">
                  {{ $t("dis.been_sent") }}
                  <!-- ({{ $t('dis.mail') }}:1234) -->
                </span>
                <span
                  v-else-if="
                    detail.crm_test_drive_order?.flag_mail_sent_user == 2
                  ">
                  <span style="color: #f31c12">{{ $t("dis.fail_send") }}</span>
                  <a-button
                    type="link"
                    style="margin-left: 8px"
                    @click="resetEmailEvent('user')"
                    >{{ $t("dis.fagain_send") }}</a-button
                  >
                </span>
              </span>
        ...
        <div class="panel-content desc-container">
          ...
          <a-row class="desc-detail">
            ...
            <a-col :xs="24" :sm="12" :lg="8" class="detail-item">
              ...
              <span class="value">
                <span v-if="storeDetail.business_time">
                  {{ $t("dis.morning") }}:
                  {{ storeDetail.business_time?.time.morning.begin }} -
                  {{ storeDetail.business_time?.time.morning.end }}
                  {{ $t("dis.afternoon") }}:
                  {{ storeDetail.business_time?.time.afternoon.begin }} -
                  {{ storeDetail.business_time?.time.morning.end }}
                </span>
                <span v-else> - </span>
              </span>
            ...
            <a-col :xs="24" :sm="12" :lg="8" class="detail-item">
              ...
              <span class="value">
                <a :href="storeDetail.official_website"
                  >{{ storeDetail.official_website || "-" }}
                </a>
              </span>
            </a-col>
          </a-row>
              ...
              <a-col
                ...
                <div class="value">
                  ...
                  <template v-if="$2.type == 0">
                    <div
                      :class="[$2.value == 'phone' ? 'phone-hover' : '']"
                      class="content-one">
                      <span
                  ...
                  <template
                    v-else-if="
                      $2.type == 2 &&
                      (msgForm['source_type'] === 1 ||
                        msgForm['source_type'] === 4)
                    ">
                    <div class="select-box">
                      ...
                      <a-select
                        :class="[
                          msg[index1].list[index2].onFocus ? '' : 'select-tab',
                        ]"
                        v-model:value="msgForm[$2.value]"
                        style="width: 80%"
                  ...
                  <template v-else-if="$2.type == 2.1">
                    <div class="select-box">
                      ...
                      <span
                        ...
                        v-else-if="!msg[index1].list[index2].onFocus">
                        {{
                          $Util.CRMTestDriveMotoExpMapFilter(
                            msgForm[$2.value],
                            $i18n.locale
                          )
                        }}
                  ...
                  <template v-else-if="$2.type == 2.4">
                    <div class="select-box">
                      ...
                      <span
                        ...
                        v-else-if="!msg[index1].list[index2].onFocus"
                        >{{
                          $Util.CRMRidingFilter(msgForm[$2.value], $i18n.locale)
                        }}</span
                      >
                      <a-select
                        :class="[
                  ...
                  <template v-else-if="$2.type == 3">
                    <div class="select-box">
                      ...
                      <span
                        ...
                        v-else-if="!msg[index1].list[index2].onFocus"
                        >{{
                          $Util.CRMCustomerIndustryFilter(
                            msgForm[$2.value],
                            $i18n.locale
                          )
                        }}</span
                  ...
                  <template v-else-if="$2.type == 4">
                    <div class="intent cursor content-one" @click="intentEvent">
                      <img
                        ...
                        alt="" />
                      <span
                        style="margin-left: 3px; width: 120px; display: block">
                        <span>
                          {{
                            $Util.CRMOrderIntentFilter(
                              msgForm[$2.value],
                              $i18n.locale
                            )
                          }}
          ...
          <a-tabs v-model:activeKey="activeKey">
            ...
            <a-tab-pane key="ContacPerson" :tab="$t('crm_t.contact_customer')">
              <CRMContact
                ...
                :flagOWN="
                  trackMemberDetail != null
                    ? trackMemberDetail.type ===
                      Core.Const.CRM_TRACK_MEMBER.TYPE.OWN
                    : false
                "
                ref="CRMContact">
                <CustomerAdd
    ...
    <a-modal v-model:visible="intentVisible" title="意向度修改" @ok="handleOk">
      <a-row class="model-intent">
        ...
        <a-col
          :md="24"
          class="intent-input"
          :class="{ required: intentionName == '10' }">
  ...
  methods: {
    ...
    storeFetch(params = {}) {
      Core.Api.Store.detail({
        ...
        .then((res) => {
          ...
          this.storeDetail.business_time = JSON.parse(
            this.storeDetail.business_time
          );
    ...
    setDetailPortraitToList(dataObj, arrData) {
      ...
      arrData.forEach((item, index) => {
        if (
          dataObj["crm_customer_portrait"] ||
          dataObj["crm_customer_portrait"][item.key]
        ) {
    ...
    msgChange(type) {
      ...
      if (type == "other_brand_model") {
        if (
          ...
          this.msgForm[type] =
            this.msgForm[type + "1"] + "-" + this.msg[1].list[5].value2;
          var othParms = {
            id: this.detail.crm_customer_portrait?.id,
            [`${type}`]: this.msgForm[type],
          };
