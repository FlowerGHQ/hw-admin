        ...
        <div class="gray-panel">
            ...
            <template v-else>
                ...
                <div class="desc-zh-container">
                                        ...
                                        <div :class="[$2.value == 'phone' ? 'phone-hover' : '']" class="content-one"><span
                                            ...
                                            <template v-if="$2.value == 'phone'">
                                                <a-button type="link"
                                                    ...
                                                    class="switch" @click="handleChecking()"><i
                                                        class="icon i_eyes" /></a-button>

                                                <CustomerSelect :radioMode="true" :phone="msgForm[$2.value]"
                                                    :check-mode="false" :select-customer="true" btn-class="select-item-btn"
                                                    btnType='link' :btnText="$t('crm_c.rechecking')">
                                                    {{ $t('crm_c.rechecking') }}
                                                </CustomerSelect>
                                            </template>
                                        </div>
                    ...
                    <div class="tabBox">
                        <AddTab :tabList="carTrackList" :targetListStr="tabData[0].value" @addStr="addCarTrackStr"
                            @delStr="deleteCarTrackStr" :keyValue="tabData[0].key" />
                    </div>
                    ...
                    <div class="tabBox">
                        ...
                        <AddTab :tabList="carHabitsList" :targetListStr="tabData[1].value" @addStr="addCarTrackStr"
                            @delStr="deleteCarTrackStr" :keyValue="tabData[1].key" />
                    </div>
                    ...
                    <div class="tabBox">
                        <AddTab :tabList="carConcensList" :targetListStr="tabData[2].value" @addStr="addCarTrackStr"
                    ...
                    <a-tabs v-model:activeKey="activeKey">
                        ...
                        <a-tab-pane key="TestDriveList" :tab="$t('crm_d.list')">
                            <CRMTestDrive v-if="id > 0" :detail="detail" :customerId="detail.id" ref="CRMTestDrive">
                                <a-button type="primary" @click="routerChange('test-drive')"
                                    v-if="$auth('crm-customer.save')">{{ $t('crm_d.save') }}</a-button>
                            </CRMTestDrive>
                        </a-tab-pane>
                        <!-- 联系人 tab -->
                        <a-tab-pane key="ContacPerson" :tab="$t('crm_t.contact_customer')">
                            <CRMContact v-if="id > 0" :detail="detail" :targetId="detail.id"
                                :targetType="Core.Const.CRM_TRACK_MEMBER.TARGET_TYPE.CUSTOMER"
                                :flagOWN="trackMemberDetail != null ? trackMemberDetail.type === Core.Const.CRM_TRACK_MEMBER.TYPE.OWN : false"
                                ref="CRMContact">
                                <CustomerAdd :btnText="$t('crm_c.add')" :targetId="detail.id"
                                    :targetType="Core.Const.CRM_TRACK_RECORD.TARGET_TYPE.CUSTOMER"
                                    :groupId="detail.group_id" @select="getCRMContactList" />
                            </CRMContact>
                        </a-tab-pane>
                        <!-- 商机 tab -->
                        <a-tab-pane key="Opportunity" :tab="$t('crm_b.new_bo')">
                            <CRMBo v-if="id > 0" :detail="detail" :customerId="detail.id" ref="CRMBo">
                                <a-button type="primary" @click="routerChange('add-crm-bo')" v-if="$auth('crm-bo.save')"><i
                                        class="icon i_add" />{{ $t('crm_b.save') }}</a-button>
                            </CRMBo>
                        </a-tab-pane>
                        <!-- 合同 -->
                        <a-tab-pane key="ContractList" :tab="$t('crm_o.list')">
                            <CRMOrder v-if="id > 0" :detail="detail" :customerId="detail.id" ref="CRMOrder">
                                <a-button type="primary" @click="routerChange('add-order')"
                                    v-if="$auth('crm-order.save')"><i class="icon i_add" />{{ $t('crm_o.save') }}</a-button>
                            </CRMOrder>
                        </a-tab-pane>
                    </a-tabs>
                </div>
                ...
                <div class="tabs-container">
                    <a-tabs v-model:activeKey="tabActiveKey">
                        <a-tab-pane key="CustomerSituation" :tab="$t('crm_c.team_members')"
                            ...
                            <Group v-if="id > 0" :targetId="id"
                                :targetType="Core.Const.CRM_TRACK_MEMBER.TARGET_TYPE.CUSTOMER" :detail="detail"
                                ref="Group" />
                        </a-tab-pane>
            ...
            <div class="form-item required">
                ...
                <div class="value">
                    <a-tree-select class="CategoryTreeSelect" v-model:value="group_id" :placeholder="$t('def.select')"
                        ...
                        @blur="getUserData('')" tree-default-expand-all />
                </div>
            </div>
            <div class="form-item required">
                ...
                */
                ...
                },
                {
                    ...
                    list: [
                        ...
                        { key: "摩托车型号", value: 'moto_model', type: 1, onFocus: undefined },
                        { key: "是否有驾照", value: 'driver_license', type: 2.1, onFocus: false },
                        { key: "骑行经验", value: 'ride_exp', type: 2.4, onFocus: false },
                        { key: "其他驾驶工具", value: 'other_brand_model', value1: '', value2: '', type: 5, onFocus: false },
                    ]
                },
