    ...
    <div id="CustomerList">
            ...
            <div class="title-container">
                ...
                <div class="btns-area">
                    <a-button type="primary" @click="routerChange('edit')" v-if="$auth('crm-customer.save')"><i
                            class="icon i_add" />{{ operMode === 'private' ? $t('c.save') : $t('crm_c.new_pool_c')
                            }}</a-button>
                </div>
            </div>
            <div class="search-container">
                <a-row class="search-area">
                    ...
                    <a-col :xs='24' :sm='24' :xl="8" :xxl='6' class="search-item">
                        ...
                        <div class="value">
                            <a-input :placeholder="$t('def.input')" v-model:value="searchForm.phone"
                                @keydown.enter='handleSearch' />
                        </div>
                    </a-col>
                    <a-col :xs='24' :sm='24' :xl="8" :xxl='6' class="search-item" v-if="show">
                        ...
                        <div class="value">
                            <a-select v-model:value="searchForm.type" :placeholder="$t('def.select')"
                                @change="handleSearch">
                                <a-select-option v-for="item of CRM_TYPE_MAP" :key="item.key" :value="item.value">{{ lang
                    ...
                    <a-col :xs='24' :sm='24' :xl="8" :xxl='6' class="search-item" v-if="show">
                        ...
                        <div class="value">
                            <a-select v-model:value="searchForm.create_user_id" show-search :placeholder="$t('def.input')"
                                ...
                                :not-found-content="null" @search="handleCreateUserSearch">
                    ...
                    <a-col :xs='24' :sm='24' :xl="8" :xxl='6' class="search-item" v-if="show">
                        ...
                        <div class="value">
                            <a-select v-model:value="searchForm.level" :placeholder="$t('def.select')"
                                @change="handleSearch">
                                <a-select-option v-for="item of CRM_LEVEL_MAP" :key="item.key" :value="item.value">{{ lang
                                    === 'zh' ? item.zh : item.en }}</a-select-option>
                            </a-select>
                        </div>
                    </a-col>
                    ...
                    <template #bodyCell="{ column, text, record }">
                        ...
                        <template v-if="column.key === 'phone'">
                            <div v-if="text !== ''" class="phone-hover">
                                ...
                                <a-button type="link" v-if="!record.flag_eyes" class="switch"
                                    @click="handleChecking(record)"><i class="icon i_eyes" /></a-button>
                            </div>
                            <template v-else>
                                {{ text || '-' }}
                            </template>
                        ...
                        <template v-if="column.key === 'operation'">
                            ...
                            <a-button type="link" @click="routerChange('detail', record)"
                                v-if="$auth('crm-customer.detail')"><i class="icon i_detail" />{{ $t('def.detail')
                                }}</a-button>
        ...
        <a-modal v-model:visible="batchGroupShow" :title="$t('crm_c.distribute_customer')" :after-close='handleBatchClose'>
            <div class="form-item required">
                <div class="key">{{ $t('crm_group.name') }}：</div>
                <div class="value">
                    <a-tree-select class="CategoryTreeSelect" v-model:value="batchForm.group_id"
                        :placeholder="$t('def.select')" :dropdown-style="{ maxHeight: '412px', overflow: 'auto' }"
                        :tree-data="groupOptions" @change="getUserData('')" tree-default-expand-all />
                </div>
            </div>
            <template #footer>
