            ...
            <div class="title-container">
                ...
                <div class="btns-area">
                    <a-button type="primary" @click="routerChange('edit')"
                        v-if="!$auth('ADMIN') && $auth('repair-order.save')"><i
                            class="icon i_add" />{{ $t('r.repair_create') }}</a-button>
                </div>
            </div>
            ...
            <div class="search-container">
                <a-row class="search-area">
                    ...
                    <a-col :xs='24' :sm='24' :xl="8" :xxl='6' class="search-item" v-if="$auth('ADMIN')">
                        ...
                        <div class="value">
                            <a-select v-model:value="searchForm.distributor_id"
                                :placeholder="$t('search.select_distributor')" @change="handleSearch" show-search
                                option-filter-prop="children">
                                <a-select-option v-for="distributor of distributorList" :key="distributor.id"
                                    :value="distributor.id">{{ distributor.name }}</a-select-option>
                            </a-select>
                        </div>
                    </a-col>
                    <a-col :xs='24' :sm='24' :xl="8" :xxl='6' class="search-item" v-if="$auth('ADMIN', 'DISTRIBUTOR')">
                        ...
                        <div class="value">
                            <a-select v-model:value="searchForm.agent_id" @change='handleSearch' show-search
                                option-filter-prop="children"
                                :placeholder="searchForm.distributor_id ? $t('search.select_agent') : $t('search.select_d_first')"
                                :disabled="!searchForm.distributor_id">
                                <a-select-option v-for="agent of agentList" :key="agent.id" :value="agent.id">{{ agent.name
                                }}</a-select-option>
                            </a-select>
                        </div>
                    </a-col>
                    <a-col :xs='24' :sm='24' :xl="8" :xxl='6' class="search-item"
                        ...
                        <div class="value">
                            <a-select v-model:value="searchForm.store_id" @change='handleSearch' show-search
                                option-filter-prop="children"
                                :placeholder="searchForm.agent_id ? $t('search.select_store') : $t('search.select_a_first')"
                                :disabled="!searchForm.agent_id">
                                <a-select-option v-for="item of storeList" :key="item.id"
                ...
                <a-table :columns="tableColumns" :data-source="tableData" :scroll="{ x: true }"
                    ...
                    <template #bodyCell="{ column, text, record }">
                        ...
                        <template v-if="column.dataIndex === 'priority'">
                            <div class="status status-bg status-tag smell"
                        ...
                        <template v-if="column.key === 'audit'">
                            <a-button type='link' @click="handleModalShow(record.id, 'audit')"
                                v-if="(record.status == STATUS.SETTLEMENT ||
                                    record.status == STATUS.DISTRIBUTOR_AUDIT_SUCCESS || record.status == STATUS.SETTLEMENT_DISTRIBUTOR) && record.service_type == 1 && $auth('repair-order.audit')"><i
                                    class="icon i_audit" />{{ $t('n.audit') }}</a-button>
                        </template>
                        <template v-if="column.key === 'redit'">
                            <a-button type='link' @click="routerChange('edit', record)"
                                v-if="record.status == STATUS.AUDIT_FAIL && $auth('repair-order.save')"><i
                                    class="icon i_edit" />{{ $t('def.edit') }}</a-button>
                        </template>
                        <template v-if="column.key === 'invoice'">
                            <a-button type='link' @click="handleModalShow(record.id, 'audit')"
                                v-if="record.status == STATUS.DISTRIBUTOR_WAREHOUSE && $auth('repair-order.audit')"><i
                                    class="icon i_audit" />{{ $t('n.audit') }}</a-button>
                        </template>
                        <template v-if="column.key === 'fault'">
                            <a-button type='link' @click="handleModalShow(record.id, 'fault')" v-if="(record.status == STATUS.FAULT_ENTITY_AUDIT || record.status == STATUS.AUDIT_SUCCESS) &&
                                record.service_type == 1 && $auth('repair-order.save-to-invoice')"><i
                                    class="icon i_s_warehouse" />{{ $t('n.storage') }}</a-button>
                        </template>
                    </template>
                </a-table>
            </div>
            ...
            <a-modal v-model:visible="modalShow" :title="$t('n.storage')" :after-close='handleModalClose'>
                <div class="modal-content">
                    <div class="form-item required">
                        ...
                        <div class="value">
                            <a-select v-model:value="faultForm.warehouse_id" :placeholder="$t('def.select')" show-search
                                option-filter-prop="children">
...
<style lang="less" scoped>// #RepairList {}</style>
