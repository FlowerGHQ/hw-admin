        ...
        <a-collapse v-model:activeKey="activeKey" ghost expand-icon-position="right">
            ...
            <a-collapse-panel key="affirm" :header="$t('r.problem')" class="gray-collapse-panel">
                <div class="panel-content affirm">
                    <div class="title">
                        <i class="icon i_warning" />{{ $t('n.all_total') }}&nbsp;{{ faultSelect.length }}&nbsp;{{
                            $t('r.faults')
                        }}
                    </div>
                    <a-checkbox-group class="fault_select" :value="faultSelect">
                        <a-checkbox
                            v-for="(value, key) of faultMap"
                            :key="key"
                            :value="key"
                            ...
                            <template #bodyCell="{ column, record, index, text }">
                                ...
                                <template v-if="column.key === 'type'">
                                    <a-select
                                        ...
                                        placeholder="维修类型"
                                        @change="handleRepairTypeChange(record)"
                                        style="width: 100px"
                                    >
                                        <a-select-option v-for="item of repairTypeMap" :key="item.key" :value="item.key"
                                            >{{ item[$i18n.locale] }}
                                        </a-select-option>
                                    </a-select>
                                </template>
                                ...
                                <template v-if="column.dataIndex === 'bad'">
                                    <template
                                        v-if="
                                            record.type === REPAIR_TYPE.REPLACE &&
                                            detail.service_type === SERVICE_TYPE.IN_REPAIR_TIME
                                        "
                                    >
                                        <template v-if="warehouseFailList.length">
                                            <a-select
                                                v-model:value="record.recycle_warehouse_id"
                                                :placeholder="$t('def.select')"
                                                style="width: 120px"
                                            >
                                                <a-select-option
                                                    v-for="item of warehouseFailList"
                                                    :key="item.id"
                                                    :value="item.id"
                                                    >{{ item.name }}
                                                </a-select-option>
                                        ...
                                        <a-button v-else type="link" @click="routerChange('warehouse')"
                                ...
                                <template v-if="column.dataIndex === 'new'">
                                    <template v-if="record.type !== REPAIR_TYPE.TRANSFER">
                                        <template v-if="record.warehouse_out_list.length">
                                            ...
                                            >
                                                <a-select-option
                                                    v-for="item of record.warehouse_out_list"
                                                    :key="item.id"
                                                    :value="item.id"
                                                >
                                                    <span
                                            ...
                                            <template v-if="needPurchase(record)">
                                                ...
                                                <a-button
                                                    ...
                                                    ><i class="icon i_goods" />{{ $t('r.purchase') }}</a-button
                                                >
                                            </template>
                                        </template>
                                        <a-button v-else type="link" @click="routerChange('warehouse')">{{
                                ...
                                <template v-if="column.dataIndex === 'operation'">
                                    <a-button type="link" class="danger" @click="handleFailItemDelete(index, fault)"
        ...
        handleFaultSubmit() {
            ...
            for (const fault of this.faultSelect) {
                ...
                for (const item of this.failData[fault]) {
                    ...
                    if (!item.is_stock && !item.change_to_transfer) {
                        // 仓库货量不足
