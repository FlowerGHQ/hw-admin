            ...
            <div class="title-container">
                ...
                <div class="btns-area" v-if="false">
                    <a-upload name="file" class="file-uploader" :file-list="upload.fileList" :action="upload.action"
                        :show-upload-list="false" :headers="upload.headers" :data="upload.data" accept=".xlsx,.xls"
                        @change="handleMatterChange">
                        <a-button type="primary" ghost class="file-upload-btn">
                            <i class="icon i_add" />{{ $t("i.import") }}
                        </a-button>
                    </a-upload>
                    <a-button class="m-l-10" type="primary" @click="routerChange('edit')" v-if="$auth('crm-order.save')"><i
                            class="icon i_add" />{{ $t("crm_o.save") }}</a-button>
                </div>
            </div>
            <div class="tab-box">
                <a-tabs v-model:activeKey="activeKey" @change="tabChange">
                    <a-tab-pane v-for="item in tabList" :key="item.value"
                        :tab="lang == 'zh' ? item.zh : item.en"></a-tab-pane>
                </a-tabs>
            </div>
            <div class="search">
                <a-row class="row-detail">
                    <a-col :xs="24" :sm="24" :xl="8" :xxl="6" class="row-item">
                        ...
                        <div class="value">
                            ...
                            <a-select v-model:value="searchForm.source_type" :placeholder="$t('def.select')">
                                ...
                                <a-select-option v-for="item of SOURCE_TYPE" :key="item.key" :value="item.value">{{ lang ===
                                    "zh" ?
                    ...
                    <a-col :xs="24" :sm="24" :xl="8" :xxl="6" class="row-item">
                        ...
                        <!-- 客户名称 -->
                        <div class="value">
                            <a-input :placeholder="$t('def.email_phone_name')" v-model:value="searchForm.customer_name"
                                @keydown.enter="handleSearch" />
                        </div>
                    </a-col>
                    <a-col :xs="24" :sm="24" :xl="8" :xxl="6" class="row-item" v-if="show">
                        ...
                        <!-- 负责人 -->
                        <div class="value">
                            <a-select v-model:value="searchForm.own_user_id" show-search :placeholder="$t('def.input')"
                                :default-active-first-option="false" :show-arrow="false" :filter-option="false"
                                :not-found-content="null" allowClear @search="handleOwnUserSearch">
                                <a-select-option v-for="item in ownUserOptions" :key="item.user_id" :value="item.user_id">
                                    {{ item.name }}
                                </a-select-option>
                            </a-select>
                        </div>
                    ...
                    <a-col :xs="24" :sm="24" :xl="8" :xxl="6" class="row-item" v-if="show">
                        ...
                        <div class="value">
                            <a-select v-model:value="searchForm.paid_money_progress" :placeholder="$t('def.select')">
                                ...
                                <a-select-option v-for="item of CRM_PAID_MONEY_PROGRESS_MAP" :key="item.key"
                    ...
                    <a-col :xs="24" :sm="24" :xl="8" :xxl="6" class="row-item" v-if="show">
                        ...
                        <div class="value">
                            <a-select v-model:value="searchForm.payment_type" :placeholder="$t('def.select')">
                                ...
                                <a-select-option v-for="item of PAYMENT_TYPE" :key="item.key" :value="item.value">{{ lang
                                    === "zh" ?
                                    item.zh : item.en }}</a-select-option>
                            </a-select>
                        </div>
                    ...
                    <a-col :xs="24" :sm="24" :xl="8" :xxl="6" class="row-item" v-if="show">
                        ...
                        <div class="value">
                            ...
                            <addressCascader ref="addressRef" v-model:value="areaMap" :def-area='showArea'
                    ...
                    <a-col :xs="24" :sm="24" :xl="8" :xxl="6" class="row-item" v-if="show">
                        ...
                        <div class="value">
                            <a-select v-model:value="searchForm.group_id" :placeholder="$t('def.select')" allowClear>
                                ...
                                <a-select-option v-for="item of regionsList" :key="item.id" :value="item.value">{{ item.name
                                }}</a-select-option>
                            </a-select>
                        </div>
                    </a-col>
                ...
                <a-table :columns="tableColumns" :data-source="tableData" :scroll="{ x: true }" :pagination="false"
                    ...
                    <template #bodyCell="{ column, text, record }">
                        ...
                        <template v-else-if="column.key === 'status'">
                            <span class="before-icon" :style="{ background: $Util.orderManageStatus(text)?.color }"
                                v-if="text"></span>{{
                        ...
                        <template v-else-if="column.key === 'operation'">
                            ...
                            <a-button type="link" @click="routerChange('detail', record)"
                                v-if="$auth('crm-order.detail')"><i class="icon i_detail" />{{ $t("def.detail")
                                }}</a-button>
                        </template>
                        <template v-else>
                            {{ text || '-' }}
