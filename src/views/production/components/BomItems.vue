            ...
            <a-table :columns="tableColumns" :data-source="addMode ? addData : tableData" :scroll="{ x: true }"
                ...
                <template #bodyCell="{ column, text, record }">
                    <template v-if="column.key === 'detail'">
                        <div class="table-img">
                            <a-image :width="30" :height="30" :src="$Util.imageFilter(record.target.logo)" fallback='无'/>
                    ...
                    <template v-if="column.dataIndex === 'amount'">
                        <template v-if="record.edit_show || editShow || addMode">
                            <a-input-number v-model:value="record.amount" style="width: 120px;"
                                :min="1" :precision="0" placeholder="请输入需求数量" /> {{record.unit || '件'}}
                        </template>
                        <template v-else>
                            {{ text }} {{record.unit || '件'}}
                        </template>
                    ...
                    <template v-if="column.dataIndex === 'comment'">
                        <template v-if="record.edit_show || editShow || addMode">
                            <a-textarea v-model:value="record.comment" style="width: 240px;" placeholder="请输入材料备注"
                                :auto-size="{ minRows: 1, maxRows: 2 }" :maxlength='99'/>
                        </template>
                        <template v-else>
    ...
    methods: {
        ...
        handleAddConfirm() {
            ...
            for (let i = 0; i < items.length; i++) {
                const item = items[i]
                if (!item.amount) {
                    return this.$message.warning(`请输入${Core.Util.bomTargetTypeFilter(item.target_type)}[${item.target.name}]的需求数量`)
                }
            }
            console.log('handleAddConfirm items:', items)
            Core.Api.BomItem.batchSave(items).then(() => {
